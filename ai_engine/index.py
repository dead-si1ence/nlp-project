...
class DocumentIndex:
    ...
    def addDocuments(self, 
                     ...
                     metadataList: List[Dict[str, Any]]) -> List[int]:
        """
    ...
    def search(self, 
               query: np.ndarray, 
               k: int = 5) -> Tuple[np.ndarray, np.ndarray, List[Dict[str, Any]]]:
        """
        ...
        for idx in indices[0]:
            if idx != -1 and idx in self.metadata:  # FAISS returns -1 for padded results
    ...
    def saveIndex(self, indexPath: str) -> None:
        ...
        os.makedirs(os.path.dirname(indexPath) if os.path.dirname(indexPath) else '.', exist_ok=True)
    ...
    def loadIndex(cls, indexPath: str) -> 'DocumentIndex':
        ...
        if not os.path.exists(f"{indexPath}.faiss") or not os.path.exists(f"{indexPath}.meta"):
