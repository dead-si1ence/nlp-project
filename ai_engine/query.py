...
class QueryEngine:
    ...
    def query(
        ...
        if not bestResult["answer"].strip():
            bestResult["answer"] = (
                "Could not find a specific answer in the document. Please try rewording your question."
            )

        # Prepare the response
        response = {"answer": bestResult["answer"], "score": bestResult["score"]}
    ...
    def simpleSearch(self, queryText: str, topK: int = 5) -> List[Dict[str, Any]]:
        """
